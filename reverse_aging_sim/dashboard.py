...
import numpy as np
# from .model import Simulation, default_tissues, default_config, plot_tissue_health, plot_bio_age, plot_km
...
# Run baseline (natural) & therapy scenarios
...
with col1:
    ...
    plot_tissue_health(res_nat, res_tx, death_threshold=cfg.death_threshold, title=f"Tissue Health — Natural={life_nat}, Therapy={life_tx}")
...
st.success(f"Avg lifespan (Natural): {np.mean(lifespans_nat):.1f}  |  Avg lifespan (Therapies): {np.mean(lifespans_tx):.1f}  |  Extension: {np.mean(lifespans_tx)-np.mean(lifespans_nat):.1f}")
